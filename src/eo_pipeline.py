...
from eo_engines import InferenceEngine, NullInferenceEngine, Provider  # ABC・ダミーエンジン
...
# wind_deg_to_vec / _wind_deg_to_math は eo_types.py に一元化済み。
...
class EOPipeline:
    ...
    def run_site(
        ...
        # bundle.get("plume_true") は、キーが存在して値が None の場合も None を返す。
        # np.ndarray に対して "or" を使うと bool(arr) が呼ばれ、
        # 要素数 != 1 の配列で ValueError が発生するため、明示的に is None で判定する。
        plume_val  = bundle.get("plume_true")
        plume_true = plume_val if plume_val is not None else np.zeros_like(mbsp)
...
def test_plume_true_is_none() -> None:
    """
    plume_true が None のとき zeros_like にフォールバックすることを確認する。

    修正前: bundle.get("plume_true") or np.zeros_like(mbsp)
            → ndarray に or を適用すると ValueError が発生する場合がある。
    修正後: val if (val := bundle.get("plume_true")) is not None else np.zeros_like(mbsp)
            → None 判定を明示化し ValueError を回避する。
    """
    print("\n" + "="*55)
    print("  TEST-3: plume_true None フォールバック")
    print("="*55)

    from eo_simulator import PlumeSimulator
    pipeline = EOPipeline(provider=PlumeSimulator())

    # plume_true=None のバンドルを作るため、最小の合成データから plume_true を除去
    sim    = PlumeSimulator()
    bundle = sim.generate(Q=3000.0, wind_speed=4.0, wind_deg=120.0, seed=0)
    bundle["plume_true"] = None  # type: ignore[assignment]

    r = pipeline.run_site(_SAMPLE_REGISTRY[0])
    assert r["plume_true"] is not None, "plume_true が None のまま"
    assert r["plume_true"].shape == r["mbsp"].shape, "フォールバック shape 不一致"
    print("  plume_true=None → zeros_like → PASS")


def test_inference_engine_protocol() -> None:
    """InferenceEngine プロトコルの差し込みテスト。"""
    print("\n" + "="*55)
    print("  TEST-4: InferenceEngine 差し込みプロトコル")
    print("="*55)
