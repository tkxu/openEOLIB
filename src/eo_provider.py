...
try:
    from science.provenance import DatasetProvenance, make_input_hash
    from science.cache      import ScienceCacheStore
    _SCIENCE_AVAILABLE = True
except ImportError:
    _SCIENCE_AVAILABLE = False
    warnings.warn(
        "science パッケージが見つかりません。cache/provenance は無効化されます。\n"
        "  science/provenance.py と science/cache.py を配置してください。",
        stacklevel=2,
    )
...
class OpenEOClient:
    ...
    def _connect(self) -> None:
        # 呼び出し元 get() がすでに _lock を保持しているため、
        # このメソッド自身は再取得しない。
...
class Sentinel2Fetcher:
    ...
    def fetch(
        ...
        """
        target_spectral = S2_BAND_SETS.get(self.band_set, S2_BAND_SETS["swir_only"])
        ...
        try:
            ...
            cube = cube.resample_spatial(resolution=RES_S2_M, projection="EPSG:4326")
            scl  = cube.band("SCL")
            cube = cube.mask((scl == 0) | (scl == 10))
            cube = (
                cube
            ...
            warnings.warn(f"[S2] 取得エラー: {e}", stacklevel=2)
            return None

        # バンド数チェック: spectral + SZA + VZA の最低限が揃っているか確認する
        n_spectral = len(target_spectral)
        n_expected = n_spectral + 2
        if data.shape[0] < n_expected:
            warnings.warn(
                f"[S2] バンド数不足 (取得={data.shape[0]}, 期待={n_expected})",
                stacklevel=2,
            )
            return None

        bands = {b: np.clip(data[i], 1e-4, None) for i, b in enumerate(target_spectral)}
        sza   = data[n_spectral]
        ...
        amf = np.where(np.isfinite(amf), amf, np.nan)
        ...
        return {
            **bands,
            ...
            "scene_id": f"S2_{dt.strftime('%Y%m%d')}_{lat:.2f}N_{lon:.2f}E",
        }
...
class DataFetcher:
    ...
    def __init__(
        ...
        self._cache = (
            ...
            if _SCIENCE_AVAILABLE and cache_dir
            else None
        )
    ...
    def fetch(self, lat: float, lon: float, dt: datetime) -> Dict:
        ...
        # --- キャッシュ参照 ---
        cache_key = None
        if _SCIENCE_AVAILABLE and self._cache is not None:
